...
export function HookAnalysisCard({ data }: HookAnalysisCardProps) {
  ...
  const colors = effectivenessColors[data.effectiveness];
    ...
    <div className="glass-card p-6">
      ...
      <div className="grid grid-cols-2 gap-4 mb-6">
        ...
        <div className="p-3 rounded-lg bg-slate-800/30">
          ...
          <div className={`text-lg font-semibold capitalize ${transitionColors[data.transitionQuality]}`}>
            {data.transitionQuality}
          </div>
        </div>
      </div>
              ...
              {expandedSection === section.id && (
                <div className="mt-2 ml-4 p-4 rounded-lg bg-slate-900/50 border border-slate-700/30">
                  ...
                  <div className="text-xs text-slate-400 mb-2">
                    Duration: {Math.round(section.duration / 60)}:{String(section.duration % 60).padStart(2, '0')} 
      ...
      {(data.logicalGaps.length > 0 || data.redundancies.length > 0) && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {data.logicalGaps.length > 0 && (
